...
class FishProcess(threading.Thread):
    ...
    def run(self):
        while self.fish.lifespan > 0:
            print(
                f"Fish {self.fish_id} ({self.fish.gender}) is swimming. Lifespan: {self.fish.lifespan}"
            )
...
class Aquarium:
    ...
    def start_simulation(self):
        breeding_thread = BreedingProcess(
            self.male_queue, self.female_queue, self.offspring_queue
        )
        ...
        for i in range(self.male_queue.qsize() + self.female_queue.qsize()):
            fish = (
                self.offspring_queue.get()
                if i >= self.male_queue.qsize()
                else self.male_queue.get()
            )
